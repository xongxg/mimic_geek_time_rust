...
enum WriteHelloFile {
    Init(String),
    // AwaitingCreate(Pin<Box<dyn Future<Output = Result<fs::File, Error>>>>),
    // AwaitingWrite(Pin<Box<dyn Future<Output = Result<(), Error>>>>),
    ...
    AwaitingWrite(Pin<Box<AwaitingWriteData>>),
    Done,
}
...
struct AwaitingWriteData {
    fut: Pin<Box<dyn Future<Output = Result<(), std::io::Error>>>>,
    file: fs::File,
}
...
//                         let wd = Box::pin(AwaitingWriteData { fut, file: v });
//                         *self = WriteHelloFile::AwaitingWrite(wd);
//                     }
...
//                 WriteHelloFile::AwaitingWrite(fut) => match fut.fut.as_mut().poll(cx) {
//                     Poll::Ready(result) => {
...
//                 Poll::Ready(Ok(mut f)) => {
//                     let fut = WriteFut { f: Box::pin(f.write_all(b"hello world!")), file: f, };
//
