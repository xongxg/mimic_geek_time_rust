...
impl ServerConfig {
    pub fn load(path: &str) -> Result<Self, KvError> {
        ...
        let config: Self = toml::from_str(&config).expect("need corrected server configuration");
        Ok(config)
    }
}
...
impl ClientConfig {
    pub fn load(path: &str) -> Result<Self, KvError> {
        let config = fs::read_to_string(path)?;
        let config: Self = toml::from_str(&config).expect("need corrected client configuration");
        Ok(config)
    }
}
...
mod tests {
    ...
    fn server_config_should_be_loaded() {
        ...
        let result = ServerConfig::load("./fixtures/server.conf");
        assert!(result.is_ok());
    }
    ...
    fn client_config_should_be_loaded() {
        ...
        let result = ClientConfig::load("./fixtures/client.conf");
        assert!(result.is_ok());
    }
}
